...
export const createMockWorkbenchDataset = (): MockDataset => {
  ...
  const manageScreenshot: ScreenshotRecord = {
    id: makeId('screenshot', 'execution_1'),
    schema_version: version,
    created_at: '2026-03-25T09:56:00+08:00',
    session_id: session.id,
    event_id: makeId('event', 'manage_1'),
    kind: 'execution',
    file_path: 'mock/chart-nq-manage.svg',
    asset_url: buildPlaceholderAssetUrl(),
    caption: '持仓中再次确认买方继续响应。',
    width: 1600,
    height: 900,
    annotations: [],
    deleted_annotations: [],
  ...
  const exitScreenshot: ScreenshotRecord = {
    id: makeId('screenshot', 'exit_1'),
    schema_version: version,
    created_at: '2026-03-25T10:08:00+08:00',
    session_id: session.id,
    event_id: makeId('event', 'exit_1'),
    kind: 'exit',
    file_path: 'mock/chart-nq-exit.svg',
    asset_url: buildPlaceholderAssetUrl(),
    caption: '离场前的高位响应图。',
    width: 1600,
    height: 900,
  ...
  const contentBlocks: ContentBlockRecord[] = [
    ...
    },
    {
      ...
      block_type: 'markdown',
      title: '持仓管理记录',
      content_md: '加速段没有追单，等二次确认后继续拿，避免在第一根扩展 K 上抬高成本。',
      sort_order: 4,
      context_type: 'event',
      context_id: makeId('event', 'manage_1'),
      soft_deleted: false,
      move_history: [],
    },
    {
      id: makeId('block', 'exit_note_1'),
      schema_version: version,
      created_at: '2026-03-25T10:08:00+08:00',
      session_id: session.id,
      event_id: makeId('event', 'exit_1'),
      block_type: 'markdown',
      title: 'Exit 记录',
      content_md: '冲击上方流动性后，价格没有继续放量延伸，先把离场图存好，后续再决定是否正式平仓。',
      sort_order: 5,
      context_type: 'event',
      context_id: makeId('event', 'exit_1'),
      soft_deleted: false,
      move_history: [],
    },
    {
      id: makeId('block', 'review_draft_1'),
      schema_version: version,
      created_at: '2026-03-25T10:09:00+08:00',
      session_id: session.id,
      event_id: makeId('event', 'review_1'),
      block_type: 'markdown',
      ...
      content_md: [
        ...
        '## 计划 vs 实际 vs 结果',
        '',
        '- 原始计划：等待 VWAP reclaim 后的第一次回踩确认。',
        '- 实际执行：已记录 setup / manage / exit 图，结果仍待最终闭环。',
        '- 结果：交易尚未正式平仓。',
        '',
        '## 偏差',
        '',
        '- 盘中是否出现了与原计划不一致的执行：待补充',
        '',
        '## 下次改进',
        '',
        '- 需要继续验证的管理动作：待补充',
  ...
  const events: EventRecord[] = [
    ...
    },
    {
      ...
      title: '执行截图',
      summary: '持仓管理截图已挂到当前 Trade 目标。',
      author_kind: 'user',
      occurred_at: '2026-03-25T09:56:00+08:00',
      content_block_ids: [contentBlocks[3].id],
      screenshot_id: manageScreenshot.id,
      ai_run_id: null,
    },
    {
      id: makeId('event', 'exit_1'),
      schema_version: version,
      created_at: '2026-03-25T10:08:00+08:00',
      session_id: session.id,
      trade_id: trade.id,
      event_type: 'screenshot',
      title: '离场截图',
      summary: '离场截图已挂到当前 Trade 目标。',
      author_kind: 'user',
      occurred_at: '2026-03-25T10:08:00+08:00',
      content_block_ids: [contentBlocks[4].id],
      screenshot_id: exitScreenshot.id,
      ai_run_id: null,
    },
    {
      id: makeId('event', 'review_1'),
      schema_version: version,
      created_at: '2026-03-25T10:09:00+08:00',
      session_id: session.id,
      trade_id: trade.id,
      ...
      author_kind: 'system',
      occurred_at: '2026-03-25T10:09:00+08:00',
      content_block_ids: [contentBlocks[5].id],
      screenshot_id: null,
      ai_run_id: null,
    },
  ]
  ...
  const currentContext = {
    ...
    session_id: session.id,
    trade_id: null,
    source_view: 'session-workbench' as const,
    capture_kind: 'chart' as const,
  }
  const targetOptions = [
    {
      id: `target_session_${session.id}`,
      ...
      trade_id: null,
      label: session.title,
      subtitle: 'Session 级目标 · Realtime view',
      is_current: true,
      trade_status: null,
      trade_side: null,
      depth: 0,
      ...
      search_text: `${contract.symbol} ${period.label} ${session.title}`,
      previous_period_trade_index: null,
    },
    {
      id: `target_trade_${trade.id}`,
      target_kind: 'trade' as const,
      contract_id: contract.id,
      period_id: period.id,
      session_id: session.id,
      trade_id: trade.id,
      label: `${trade.symbol} 做多`,
      subtitle: `持仓中 · 数量 ${trade.quantity} · 开始 ${trade.opened_at}`,
      is_current: false,
      trade_status: trade.status,
      trade_side: trade.side,
      depth: 1,
      parent_target_id: `target_session_${session.id}`,
      period_label: period.label,
      session_title: session.title,
      search_text: `${contract.symbol} ${period.label} ${session.title} ${trade.symbol} ${trade.side} ${trade.status}`,
      previous_period_trade_index: null,
    },
  ]
  const targetOptionGroups = {
    current: [targetOptions[0]],
    recent: [targetOptions[1]],
    history: targetOptions,
  ...
  const dataset: WorkbenchDataset = {
    ...
    trades: [trade],
    events,
    screenshots: [screenshot, manageScreenshot, exitScreenshot],
    deleted_screenshots: [],
